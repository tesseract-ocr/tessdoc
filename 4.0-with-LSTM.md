...
3.0 version of box files can be converted for use with LSTM training by adding a tab character at end of each line. `Mark EOL` function under Edit in `Box Editor` tab of latest version of [JTessboxeditor](https://sourceforge.net/projects/vietocr/files/jTessBoxEditor/) can be used to do it automatically.
